...
public final class MoveOrdering {
    ...
    private static final com.chess.engine.classic.player.ai.MoveOrdering INSTANCE = new com.chess.engine.classic.player.ai.MoveOrdering();
    ...
    public static com.chess.engine.classic.player.ai.MoveOrdering get() {
        return INSTANCE;
    }
    ...
    private List<Move> orderImpl(final Board board,
                                 final int depth) {
        final List<com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry> moveOrderEntries = new ArrayList<>();
        for (final Move move : board.currentPlayer().getLegalMoves()) {
            ...
            if (moveTransition.getMoveStatus().isDone()) {
                ...
                moveOrderEntries.add(new com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry(move, currentValue));
            }
        }
        Collections.sort(moveOrderEntries, new Comparator<com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry>() {
            @Override
            public int compare(final com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry o1, final com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry o2) {
                return Ints.compare(o2.getScore(), o1.getScore());
            }
        });
        final List<Move> orderedMoves = new ArrayList<>();
        for(final com.chess.engine.classic.player.ai.MoveOrdering.MoveOrderEntry entry : moveOrderEntries) {
